...
async function createPublication(req, res) {
  ...
  const filePath = req.files?.file?`/uploads/research-publications/${req.files.file[0].filename}`:null;
...
async function updatePublication(req, res) {
  const { id } = req.params;
  const { title, author, abstract, publication_date, status, file_path } =
    req.body;
